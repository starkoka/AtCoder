/*
 * Nada Personal Computer Association Programming Contest 2024
 * Member : eom2357, tsunamayo123, kokastar
 * Coding : kokastar
 */
#include <bits/stdc++.h>
#include <atcoder/all>
...
int main() {
    ...
    vector<modint998244353> kaijo(101,1);
    rep(i,1,100){
        kaijo[i] = kaijo[i-1]*i;
    }

    //dp[i][j] = i番目まで選んで総和がjになる組み合わせ数
    vector<vector<vector<modint998244353>>> dp(n+1,vector<vector<modint998244353>>(m+1,vector<modint998244353>(n+1,0)));
    dp[0][0][0] = 1;
    rep(i,0,n){
        rep(j,0,m){
            rep(k,0,n){
    ...
    rep(i,0,n+1){
        modint998244353 sum = dp[n][m/2][i];
        ans += kaijo[i] * kaijo[n-i] * sum;
    }
    cout << ans.val() << nl;
}
