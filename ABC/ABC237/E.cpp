...
int main() {
    ...
    rep(i,0,m){
        ...
        cin >> u >> v;
        u--;v--;
        vec[u].emplace_back(makep(v,max(h[v]-h[u],0LL)));
        vec[v].emplace_back(makep(u,max(h[u]-h[v],0LL)));
    }
    ...
    while(!q.empty()){
        ...
        check[now.S] = true;
        chmax(ans,(h[now.S]+cur[now.S]-h[0])*-1);

        fore(i,vec[now.S]){
            if(check[i.F])continue;
            if(chmin(cur[i.F],now.F+i.S)){
                q.push(makep(cur[i.F],i.F));
            }
        }
    }
    cout << ans << nl;
