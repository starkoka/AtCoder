...
int main() {
    ...
    cin >> n;
    vi a(n);
    unordered_map<int,vi> check;
    rep(i,0,n){
        cin >> a[i];
        check[a[i]].emplace_back(i);
        a[i] %= 200;
    }
    vector<set<intp>> m(200);
    int sum = -1;
    rep(i,0,n){
        m[a[i]].insert(makep(-1*(i+1),a[i]));
        if(m[a[i]].size()>=2){
            sum = a[i];
            break;
        }
        vector<pair<int,intp>> add;
        rep(j,0,200){
            if(m[j].size()==0 || a[i]==j)continue;
            int number = (a[i]+j)%200;
            add.emplace_back(makep(number,makep(max(a[i],j),min(a[i],j))));
            if(m[number].size()>=1){
                sum = number;
                break;
            }
    ...
    rep(i,0,n)m[a[i]].insert(makep(-1*(i+1),a[i]));

    vi cvec,bvec;
    queue<int> cq,bq;
    bool flag = false; //2手に既に分かれたか
    cq.push(sum);bq.push(sum);

    while(!cq.empty() || !bq.empty()){
        if(!flag){
            int now = cq.front();
            cq.pop();bq.pop();

            auto vec = m[now].begin();
            auto f = *vec;
            vec++;
            auto s = *vec;
