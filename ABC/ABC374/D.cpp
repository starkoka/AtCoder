...
double dist(intp a, intp b){
    return sqrt((a.F-b.F)*(a.F-b.F) + (a.S-b.S)*(a.S-b.S));
}

double solve(vector<pair<intp,intp>> &vec, bitset<6> &b, vi &idx){
    double ans = 0;
    intp now = {0,0};
    rep(i,0,n){
        intp start = (b.test(idx[i]) ? vec[idx[i]].F : vec[idx[i]].S);
        intp goal = (b.test(idx[i]) ? vec[idx[i]].S : vec[idx[i]].F);

        ans += dist(now,start)/s;
        ans += dist(start, goal)/t;
        now = goal;
    }
    return ans;
}
...
void solveAtCoder(){
    cin >> n >> s >> t;
    vector<pair<intp,intp>> vec(n);
    vi idx;
    rep(i,0,n){
        cin >> vec[i].F.F >> vec[i].F.S;
        cin >> vec[i].S.F >> vec[i].S.S;
        idx.emplace_back(i);
    }
