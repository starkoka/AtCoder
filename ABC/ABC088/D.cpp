...
int main() {
    ...
    q.push(makep(1,makep(0,0)));
    vii score(h,vi(w,INT_MAX/10));
    vbb check(h,vb(w,false));
    while(!q.empty()){
        auto [s,now] = q.top();
        q.pop();
        if(check[now.F][now.S])continue;
        check[now.F][now.S] = true;
        if(now.F==h-1 && now.S==w-1)break;

        if(now.F > 0){
            now.F--;
            if(vec[now.F][now.S]=='.' && !check[now.F][now.S] && chmin(score[now.F][now.S],s+1)){
                q.push(makep(s+1,now));
            }
            now.F++;
        }
        if(now.S > 0){
    ...
    cout << max(-1,ans-score[h-1][w-1]) << nl;
}