...
int cnt;
vector<vector<intp>> check(1009,vector<intp>(1009,makep(-1,-1)));
vector<vector<vector<intp>>> tree(1009,vector<vector<intp>>(1009,vector<intp>(0)));

void dfs(intp s,intp now){
    cnt++;
    fore(i,tree[now.F][now.S]){
        if(check[i.F][i.S]!=s){
            check[i.F][i.S] = s;
            dfs(s,i);
        }
...
int main() {
        ...
        rep(j,0,w){
            ...
            rep(k,0,4){
                if(i+dx[k]>=0 && i+dx[k]<h && j+dy[k]>=0 && j+dy[k]<w){
                    tree[i][j].emplace_back(makep(i+dx[k],j+dy[k]));
                    if(vec[i+dx[k]][j+dy[k]] == '#')can = true;
                }
            }
            if(can){
                tree[i][j].clear();
    ...
    rep(i,0,h){
        rep(j,0,w){
            if(vec[i][j]=='.' && check[i][j]==makep(-1,-1)){
                cnt = 0;
                check[i][j] = makep(i,j);
                dfs(makep(i,j),makep(i,j));
                chmax(ans,cnt);
            }
        }
    }
