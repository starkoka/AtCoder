...
void dfs(int y, int x, unordered_set<int> &s){
    if(s.contains(y*100+x))return;
    s.insert(y*100+x);
    if(s.size() == k+1){
        ans++;
        s.erase(y*100+x);
    ...
    rep(i,0,4){
        if(y+idx[i].F < 0 ||y+idx[i].F >= h || x+idx[i].S < 0 || x+idx[i].S >= w)continue;
        if(v[y+idx[i].F][x+idx[i].S] == '#')continue;
        dfs(y+idx[i].F, x+idx[i].S, s);
    }
    s.erase(y*100+x);
}
