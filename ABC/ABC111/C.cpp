...
int main() {
    ...
    int n;
    cin >> n;
    vector<vector<intp>> vec(2,vector<intp>(100009));
    rep(i,0,2) {
        rep(j,0,vec[i].size()) {
            vec[i][j] = make_pair(0,j);
        }
    }
    rep(i,0,n) {
        int v;
    ...
    rep(i,0,2)sort(all(vec[i]));

    if(vec[0][vec[0].size()-1].S == vec[1][vec[0].size()-1].S) {
        int ans = max(vec[0][vec[0].size()-2].F+vec[1][vec[0].size()-1].F , vec[1][vec[0].size()-2].F+vec[0][vec[0].size()-1].F);
        cout << n-ans << nl;
    }
    else {
        cout << n - vec[0][vec[0].size()-1].F - vec[1][vec[1].size()-1].F << nl;
    }
}