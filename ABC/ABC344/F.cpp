...
int main() {
    ...
    //dp[i][j][k] = (i,j)にいてk円増やせるときの最善の(行動回数,所持金)
    dp[0][0][p[0][0]] = makep(0,0);
    rep(i,0,n) {
        rep(j,0,n) {
            if(j!=n-1) { //R移動
                for(auto [k,now] : dp[i][j]) {
                    now.F++;
                    if(now.S < r[i][j]) {
                        ll add = (r[i][j]-now.S)/k + ((r[i][j]-now.S)%k==0?0:1);
                        now.F += add;
                        now.S += add*k;
                    }
                    now.S -= r[i][j];
                    ll idx = max(k,p[i][j+1]);
                    if(dp[i][j+1].count(idx)==0) {
                        dp[i][j+1][idx] = now;
                    }
                    else if(dp[i][j+1][idx].F > now.F) {
                        dp[i][j+1][idx] = now;
                    }
                    else if(dp[i][j+1][idx].F == now.F  &&  dp[i][j+1][idx].S < now.S) {
                        dp[i][j+1][idx] = now;
                    }
                }
            }
            if(i!=n-1) { //D移動
                for(auto [k,now] : dp[i][j]) {
                    now.F++;
                    if(now.S < d[i][j]) {
                        ll add = (d[i][j]-now.S)/k + ((d[i][j]-now.S)%k==0?0:1);
                        now.F += add;
                        now.S += add*k;
                    }
                    now.S -= d[i][j];
                    ll idx = max(k,p[i+1][j]);
                    if(dp[i+1][j].count(idx)==0) {
                        dp[i+1][j][idx] = now;
                    }
                    else if(dp[i+1][j][idx].F > now.F) {
                        dp[i+1][j][idx] = now;
                    }
                    else if(dp[i+1][j][idx].F == now.F  &&  dp[i+1][j][idx].S < now.S) {
                        dp[i+1][j][idx] = now;
                    }
                }
            }
