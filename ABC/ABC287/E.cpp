...
int main() {
    int H,W,h,w,n;
    cin >> H >> W >> n >> h >> w;
    vector<vii> vec(H+1,vii(W+1,vi(n+1,0)));
    rep(i,1,H+1){
        rep(j,1,W+1){
            int a;
            cin >> a;
            vec[i][j][a]++;
            if(j!=W)vec[i][j+1] = vec[i][j];
        }
    }
    ...
    rep(i,1,H-h+2){
        rep(j,1,W-w+2){
            int count = 0;
            rep(num,1,n+1){
                int area = vec[i-1][j-1][num] + vec[i+h-1][j+w-1][num] - vec[i-1][j+w-1][num] - vec[i+h-1][j-1][num];
                if(vec[H][W][num]-area > 0)count++;
            }
            cout << count << " ";
        }
        cout << nl;
